...
Base and holder settings share one workspace. Each generator keeps its own active shape and size, while size-label visibility and magnet dimensions stay in sync between them and are remembered in this browser. Generator-specific settings and per-footprint magnet counts return to their defaults when the page reloads.
