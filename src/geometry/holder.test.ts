...
describe('holderLayout', () => {
  ...
  it('uses an adaptive staggered lattice for eight 32mm models in 2×3', () => {
    const config = { ...defaultHolderConfig(), groups: [{ id: 'models-1', quantity: 8, diameter: 32 }], maxColumns: 2, maxRows: 3 }
  ...
  it('uses two columns for a 50mm model and three for a 90mm model', () => {
    expect(holderLayout({ ...defaultHolderConfig(), groups: [{ id: 'models-1', quantity: 1, diameter: 50 }] }).unitsWide).toBe(2)
    expect(holderLayout({ ...defaultHolderConfig(), groups: [{ id: 'models-1', quantity: 1, diameter: 90 }] }).unitsWide).toBe(3)
  })
  ...
  it('packs mixed model sizes into one holder', () => {
    const config = {
      ...
      groups: [
        ...
        { id: 'models-2', quantity: 2, diameter: 40 },
      ],
    }
    const layout = holderLayout(config)
    expect(layout.slotCenters.map((slot) => slot.diameter).sort((a, b) => a - b)).toEqual([32, 32, 32, 32, 32, 40, 40])
  })

  it('fits forty 32mm models within a 7×5 box without false overflow', () => {
    const config = { ...defaultHolderConfig(), groups: [{ id: 'models-1', quantity: 40, diameter: 32 }] }
    const layout = holderLayout(config)
  ...
  it('fits a large mixed request with structured shelf packing', () => {
    const config = {
      ...defaultHolderConfig(),
      groups: [
        { id: 'models-1', quantity: 34, diameter: 32 },
        { id: 'models-2', quantity: 2, diameter: 40 },
      ],
    }
  ...
  it('keeps a full wall between every slot and the holder edge', () => {
    ...
    expect(
      layout.slotCenters.every(
        (point) =>
          Math.abs(point.x) <= layout.width / 2 - point.diameter / 2 && Math.abs(point.y) <= layout.length / 2 - point.diameter / 2,
      ),
    ).toBe(true)
  })
    ...
    for (let i = 0; i < points.length; i++) {
      for (let j = 0; j < i; j++)
        expect(Math.hypot(points[i].x - points[j].x, points[i].y - points[j].y)).toBeGreaterThanOrEqual(
          (points[i].diameter + points[j].diameter) / 2 + config.spacing - 1e-5,
        )
    }
  })

  it('reports no layout when the box constraints are too small', () => {
    const config = { ...defaultHolderConfig(), groups: [{ id: 'models-1', quantity: 2, diameter: 90 }], maxColumns: 3, maxRows: 2 }
...
describe('holderPlan', () => {
  it('keeps separate miniature groups in reusable modules', () => {
    const config = {
      ...
      groups: [
        ...
        { id: 'character', quantity: 1, diameter: 32 },
      ],
    }
  ...
  it('reduces the fitted quantity instead of blocking an undersized box', () => {
    ...
    const plan = holderPlan(config)
    expect(plan.omitted).toEqual([{ id: 'character', quantity: 1, diameter: 32 }])
  })
  ...
  it('keeps earlier groups first when the whole request cannot fit', () => {
    ...
    expect(holderPlan(config).omitted).toEqual([{ id: 'unit', quantity: 1, diameter: 32 }])
  })
  ...
  it('keeps different sizes together unless splitting is enabled', () => {
    const config = {
      ...
      groups: [
        { id: 'unit', quantity: 5, diameter: 32 },
        { id: 'character', quantity: 1, diameter: 40 },
      ],
    }
...
describe('buildHolder', () => {
  ...
  it('has a separate locating foot for every Gridfinity cell', () => {
    const { mesh } = buildHolder(wasm, { ...defaultHolderConfig(), groups: [{ id: 'models-1', quantity: 8, diameter: 32 }], maxRows: 3 })
    const { numProp, vertProperties } = mesh
