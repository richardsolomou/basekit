...
export function loadWorkspace(storage: SettingsStorage): WorkspaceState {
  try {
    ...
    let workspace =
      parsed.version === 1
        ...
        : parsed.version === 2
          ...
          : parsed.workspace
    if ([1, 2, 3, 4, 5, 6, 7, 8].includes(parsed.version as number) && typeof workspace === 'object' && workspace !== null) {
    ...
    if (![1, 2, 3, 4, 5, 6, 7, 8, WORKSPACE_VERSION].includes(parsed.version as number)) return defaultWorkspace()
